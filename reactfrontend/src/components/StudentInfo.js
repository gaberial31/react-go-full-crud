...
export default class StudentInfo extends Component {
    ...
    getMajor = () => {
        var dataAllMajor = []
        Axios.get('http://localhost:7070/major').then(result => {
    ...
    getStudentbyID() {
        ...
        Axios.get(`http://localhost:7070/student/${params.studentId}`).then(result => {
    ...
    handleDelete() {
        const { match: { params } } = this.props;
        Axios.delete(`http://localhost:7070/student/${params.studentId}`)
            .then(() => {
    ...
    handleSubmit = event => {
        ...
        const { match: { params } } = this.props;
        Axios.put(`http://localhost:7070/student/${params.studentId}`, {
